...
class Fine:
    ...
    def lift_fine_Auto(
        ...
        for fine in cls._fines:
            if (
                fine.status == Fine_Status.ACTIVE
                and fine.end_date
                and current_date >= fine.end_date
            ):
                fine.status = Fine_Status.INACTIVE
    ...
    @classmethod
    def get_fine_by_id(cls, fine_id: str) -> Fine | None:
        for fine in cls._fines:
            if fine.id_fine == fine_id:
                return fine
        return None

    @classmethod
    def get_fines_by_reader_id(cls, reader_id: str) -> List[Fine]:
        return [fine for fine in cls._fines if fine.reader_id == reader_id]
